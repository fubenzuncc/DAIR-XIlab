...
from v2x.models.model_utils import (
    ...
    inference_lidar_camera_3d_detector,
    BBoxList,
    EuclidianMatcher,
    SpaceCompensator,
    TimeCompensator,
...
class LateFusionInf(nn.Module):
    ...
    def pred(self, frame, trans, pred_filter):
        ...
        elif self.args.sensortype == "lidar_camera":
            id = frame.id["lidar"]
            camera_id = frame.id["camera"]
            logger.debug("infrastructure pointcloud_id={}, image_id={}".format(id, camera_id))
            path = osp.join(self.args.output, "inf", "lidar", id + ".pkl")
            frame_timestamp = frame["pointcloud_timestamp"]
        ...
        if self.args.sensortype == "lidar":
            ...
            point_file = frame.point_cloud(data_format="file")
            image_file = osp.join(self.args.input, "infrastructure-side", frame["image_path"])
            annos = osp.join(self.args.output, "infrastructure-side", "annos", id + ".json")
            result, _ = inference_lidar_camera_3d_detector(self.model, point_file, image_file, annos)
...
class LateFusionVeh(nn.Module):
    ...
    def pred(self, frame, trans, pred_filter):
        ...
        if self.args.sensortype == "lidar":
            ...
            id = frame.id["lidar"]
            camera_id = frame.id["camera"]
            logger.debug("vehicle pointcloud_id={}, image_id={}".format(id, camera_id))
            path = osp.join(self.args.output, "veh", "lidar", id + ".pkl")
            frame_timestamp = frame["pointcloud_timestamp"]
        ...
        if self.args.sensortype == "lidar":
            ...
            point_file = frame.point_cloud(data_format="file")
            image_file = osp.join(self.args.input, "vehicle-side", frame["image_path"])
            annos = osp.join(self.args.output, "vehicle-side", "annos", id + ".json")
            result, _ = inference_lidar_camera_3d_detector(self.model, point_file, image_file, annos)
