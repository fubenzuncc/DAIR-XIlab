...
from mmdet3d.models import build_model, build_bev_model
...
def init_model(config, checkpoint=None, device="cuda:0"):
    ...
    if 'BEVFusion' in config.model.type:
        model = build_bev_model(config.model, test_cfg=config.get("test_cfg"))
    else:
...
def inference_lidar_camera_3d_detector(model, pcd, image, ann_file):
    """Inference point+image with the monocular 3D detector.
    ...
    if box_mode_3d == Box3DMode.CAM:
        ...
        data["camera_intrinsics"] = cam_intrinsic
    elif box_mode_3d == Box3DMode.LIDAR: # lidar points to image conversion
        cam_intrinsic = np.eye(4, 4)
        cam_intrinsic[:3, :3] = np.array(img_info["calib"]["cam_intrinsic"]).reshape(3, 3)
        ...
        # lidar2img = P2 @ rect @ Trv2c
        data["lidar2img"] = lidar2img.astype(np.float32)
        data["camera_intrinsics"] = cam_intrinsic.astype(np.float32)
