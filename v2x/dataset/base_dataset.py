...
def get_annos(path, prefix, single_frame, sensortype="camera", output_dir="/tmp"):
    trans0_path = ""
    ...
    if not osp.exists(osp.join(output_dir, prefix)):
        os.mkdir(osp.join(output_dir, prefix))
    if not osp.exists(osp.join(output_dir, prefix, "annos")):
        os.mkdir(osp.join(output_dir, prefix, "annos"))
    ann_path_o = osp.join(output_dir, prefix, "annos", single_frame["image_path"].split("/")[-1].split(".")[0] + ".json")
