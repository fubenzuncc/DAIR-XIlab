...
class DAIRV2XI(DAIRV2XDataset):
    def __init__(self, path, args, split="train", sensortype="lidar", extended_range=None):
        ...
        for elem in data_infos:
            ...
            self.data.append((InfFrame(path, elem, output_dir=self.args.output), gt_label, filt))

            if sensortype in ["camera", "lidar_camera"]:
                inf_frame = self.inf_path2info[elem["image_path"]]
                get_annos(path, "infrastructure-side", inf_frame, "camera", output_dir=self.args.output)
...
class DAIRV2XV(DAIRV2XDataset):
    def __init__(self, path, args, split="train", sensortype="lidar", extended_range=None):
        ...
        for elem in data_infos:
            ...
            self.data.append((VehFrame(path, elem, output_dir=self.args.output), gt_label, filt))

            if sensortype in ["camera", "lidar_camera"]:
                veh_frame = self.veh_path2info[elem["image_path"]]
                get_annos(path, "vehicle-side", veh_frame, "camera", output_dir=self.args.output)
    ...
    def __init__(self, path, args, split="train", sensortype="lidar", extended_range=None):
        ...
        for elem in frame_pairs:
            if sensortype == "lidar":
                ...
                get_annos(path, "infrastructure-side", inf_frame, "camera", output_dir=self.args.output)
                get_annos(path, "vehicle-side", veh_frame, "camera", output_dir=self.args.output)
            elif sensortype == 'lidar_camera':
                inf_frame = self.inf_path2info[elem["infrastructure_pointcloud_path"]]
                veh_frame = self.veh_path2info[elem["vehicle_pointcloud_path"]]
                get_annos(path, "infrastructure-side", inf_frame, "camera", output_dir=self.args.output)
                get_annos(path, "vehicle-side", veh_frame, "camera", output_dir=self.args.output)

            inf_frame = InfFrame(path + "/infrastructure-side/", inf_frame, output_dir=self.args.output)
            veh_frame = VehFrame(path + "/vehicle-side/", veh_frame, output_dir=self.args.output)
...
class VICAsyncDataset(VICDataset):
    def __init__(self, path, args, split="train", sensortype="lidar", extended_range=None):
        ...
        for vic_frame, coop_labels, filt in self.data:
            inf_frame, delta_t = self.prev_inf_frame(
                vic_frame.inf_frame.id[sensortype] if sensortype != "lidar_camera" else vic_frame.inf_frame.id['lidar'],
                sensortype,
            )
    ...
    def prev_inf_frame(self, index, sensortype="lidar"):
        if sensortype in ["lidar", "lidar_camera"]:
            ...
            return (
                InfFrame(self.path + "/infrastructure-side/", prev, output_dir=self.args.output),
            ...
            get_annos(self.path, "infrastructure-side", prev, "camera", output_dir=self.args.output)
            return (
                InfFrame(self.path + "/infrastructure-side/", prev, output_dir=self.args.output),
